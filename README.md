...
> **⚠️ 测试功能,慎用**:本功能通过进程内定时器到点后以非 0 退出码(`System.exit(1)`)退出,
> 依赖面板「进程退出自动重启」机制拉起新实例。**已实机验证可用,但存在以下已知行为:**
> - 面板会把非 0 退出码判定为「启动失败」并打印 `Code de sortie : 1` / `impossible de lancer` 等警告文案(无害,面板仍会自动重启)
> - 若面板对「短时间反复异常退出」有保护机制,间隔设置过短(如 <10 分钟)可能导致面板停止自动重启、实例停在 stopped
> - 建议间隔 ≥30 分钟,并确认面板为「进程退出自动重启」模式后再启用
> - **非必需功能**:一般部署不需要开,仅在担心免费面板闲置回收时才用
